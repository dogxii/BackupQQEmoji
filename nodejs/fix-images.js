...
async function detectImageFormat(filePath) {
	...
	try {
		...
		if (
			bufferStartsWith(buffer, SIGNATURES.GIF89a) ||
			bufferStartsWith(buffer, SIGNATURES.GIF87a)
		) {
		...
		if (
			bufferStartsWith(buffer, SIGNATURES.WEBP_RIFF) &&
						...
						if (!isCorrect) {
							...
							if (file !== newName) {
								...
								try {
									...
									console.log(
										`[修复] ${file} -> .${correctExt} \t[${result.type}]`,
									);
									stats.renamed++;
								}
							}
